...
import os,sys,time,hashlib,click
try:
...
def encrypt(textfile, passcode):
    ...
    if not os.path.exists(textfile):
        ...
        def pad_file(textfile):
            ...
            return textfile
        #now filled with extra whitespace, its ready to be encrypted. Creating the new file:
    ...
    #changing the name of the original file with the add-on enc in which the encrypted data will be stored
        ...
        #opening the textfile and reading as bytes:
            ...
            with open('padded_file', 'wb') as enc:
        #opening the enc_file in the writing mode:
                enc.write(encrypted_message)
        # NOT SURE HERE WHETHER SHOUKL I PASS TEXTFILE(PADDED) OR ENC_FILE :/
    ...
    if os.path.exists(textfile):
        print('Do you want to keep the original file,  %s? (Y)es or (N)o ?' %(textfile))
        response = input('> ')
